...
export const deleteAudition = async({auditionId, userId, pathname})=>{
    try {
      ...
      await User.findByIdAndUpdate(userId, {
         ...
         $pull:{attendedEvents:deletedAudition._id}
      })
      revalidatePath(pathname)
    } catch (error) {
      console.log(error)
...
export async function addUserToEvent({auditionId, userId, pathname}){
   try {
      await connectToDB();
      
     const audition =  await Audition.findByIdAndUpdate(auditionId, {
         $push:{attendees:userId}
      })

      await User.findByIdAndUpdate(userId, {
         $push:{attendedEvents:auditionId}
      })
      revalidatePath(pathname)

      return JSON.parse(JSON.stringify(audition))
   } catch (error) {
   console.log(error)   
   }
}